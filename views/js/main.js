...
function generator(e, a) {
	var r = getAdj(e);
	var n = getNoun(a);
	var i = parseInt(Math.random() * r.length);
	var t = parseInt(Math.random() * n.length);
	var o = "The " + r[i].capitalize() + " " + n[t].capitalize();
	return o
}
function randomName() {
	var e = parseInt(Math.random() * adjectives.length);
	var a = parseInt(Math.random() * nouns.length);
...
function updatePositions() {
	...
	for (var e = items.length, a = document.body.scrollTop / 1250, r = [], n = 0; n < 5; n++)
		r.push(100 * Math.sin(a + n));
	for (var n = 0; n < e; n++)
		items[n].style.left = items[n].basicLeft + r[n % 5] + "px";
...
var makeRandomPizza = function() {
	for (var e = "", a = Math.floor(4 * Math.random()), r = Math.floor(3 * Math.random()), n = Math.floor(2 * Math.random()), i = 0; i < a; i++)
		e += ingredientItemizer(selectRandomMeat());
	for (var t = 0; t < r; t++)
		e += ingredientItemizer(selectRandomNonMeat());
	for (var o = 0; o < n; o++)
		e += ingredientItemizer(selectRandomCheese());
	return e += ingredientItemizer(selectRandomSauce()),
	e += ingredientItemizer(selectRandomCrust())
}
var pizzaElementGenerator = function(e) {
	...
	a.classList.add("randomPizzaContainer"),
	a.style.width = "33.33%",
...
var resizePizzas = function(e) {
	function a(e) {
		switch (e) {
		case "1":
			return void (i.innerHTML = "Small");
		case "2":
			return void (i.innerHTML = "Medium");
		case "3":
			return void (i.innerHTML = "Large");
		default:
	...
	function r(e, a) {
		...
		var n = e.offsetWidth;
		var i = document.getElementById("randomPizzas").offsetWidth;
		var t = n / i;
		var o = r(a);
		var s = (o - t) * i;
		return s;
	...
	function n(e) {
		for (var a = document.getElementsByClassName("randomPizzaContainer").length, n = document.getElementsByClassName("randomPizzaContainer"), i = [], t = 0; t < a; t++)
			i.push(n[t].offsetWidth + r(n[t], e) + "px");
		for (var t = 0; t < a; t++)
			n[t].style.width = i[t]
	}
	window.performance.mark("mark_start_resize");
	var i = document.getElementById("pizzaSize");
	a(e),
	n(e),
	window.performance.mark("mark_end_resize"),
	window.performance.measure("measure_pizza_resize", "mark_start_resize", "mark_end_resize");
	var t = window.performance.getEntriesByName("measure_pizza_resize");
	console.log("Time to resize pizzas: " + t[t.length - 1].duration + "ms")
};
...
document.addEventListener("DOMContentLoaded", function() {
	var e = 8
	var a = 256
	var r = document.getElementById("movingPizzas1")
	var n = window.innerHeight / a * e;
	console.log(n);
	for (var i = 0; i < n; i++) {
		var t = document.createElement("img");
		t.className = "mover",
		t.src = "images/pizza100x100.png",
		t.style.height = "73.333px",
		t.style.width = "73.333px",
		t.basicLeft = i % e * a,
		t.style.top = Math.floor(i / e) * a + "px",
		r.appendChild(t)
	}
